...
description: "Use when the user drops PDF or CSV bank/credit-card statements, or says 'process these statements' / 'make a P&L' / 'categorize these transactions' / 'reconcile this period'. Runs the full pipeline: extract (parallel Haiku) → canonicalize + categorize (parallel Sonnet) → Google Sheets workbook with formula-driven P&L (single Sonnet writer)."
---

# Bookkeeping Pipeline

You act as bookkeeper. User drop PDF statements, you produce reviewed **Google Sheets** workbook with formula-driven P&L. Do work. No clarifying questions unless step truly cannot proceed.

## Output Target: Google Sheets via Composio

Composio CLI at **`/Users/milo/.composio/composio`** (not on PATH — always use full path). All Google Sheets writes go through it.

**Before any run**, verify `googlesheets` toolkit connected:

```bash
/Users/milo/.composio/composio execute GOOGLESHEETS_SEARCH_SPREADSHEETS -d '{"query": "", "max_results": 1}'
```

If returns `"No active connection found for toolkit \"googlesheets\""`, STOP and ask user to connect:

```bash
/Users/milo/.composio/composio link googlesheets --no-wait
```
...
Agent single-company. CoA + memory live at agent root (flat). Each run get own folder under `runs/{period}/`.
...
├── chart-of-accounts.json                  # authoritative CoA (locked during a run)
...
4. (Optional) custom CoA file (xlsx / csv / inline text), vendor list, or prior Transaction Detail.

## Procedure

### Step 1 — Bootstrap context + lock CoA

1. **Load existing state:**
   - `context/bookkeeping-context.md` — brief. If missing, stop and ask user to run `define-bookkeeping-context` first (or ask to do inline).
   - `config/context-ledger.json` — accounts, suspense code.
   - `config/chart-of-accounts.json` — authoritative CoA. If exists, **LOCK for this run.**
   - `config/prior-categorizations.json` — vendor → GL memory.
   - `config/party-rules.json` — exact-match rules.

2. **First-run bootstrap (only if `config/chart-of-accounts.json` absent):**
   - If user provided CoA file (xlsx/csv), parse (openpyxl for xlsx) into `config/chart-of-accounts.json` as `[{code, name, type, statementSection}]`.
   - If user described CoA inline, structure that way.
...
3. **Lock CoA for rest of run.** Treat `config/chart-of-accounts.json` immutable until Step 7. If transaction can't categorize, send to Suspense — NEVER invent new GL code.
...
If the canonical party exactly matches a key in the work packet's party_rules, assign that GL code.
- confidence: 1.00
- source: "rule"

**2. prior_categorizations fuzzy match**
If the canonical party fuzzy-matches a key in prior_categorizations (token set ratio ≥ 0.85) AND the stored GL code is in the chart_of_accounts, use it.
- confidence: 0.95
- source: "prior_year"

**3. Your reasoning against the chart_of_accounts**
Look at description, canonical party, amount, and the account type. Pick the best GL code from the chart_of_accounts.
...
**DO NOT assign GL code 9000 (Internal Transfer).** You cannot see other accounts' transactions. If a transaction looks like an inter-account transfer (e.g., "Chase Autopay", "Transfer to Checking"), send it to Suspense unless it exactly matches a party_rule.

**DO NOT invent GL codes** not present in the chart_of_accounts.
...
| Cross-account transfers found | Count and total absolute amount | Log: "Found {N} transfer pair(s) totaling ${X} — tagged GL 9000" |
...
The Composio CLI is at /Users/milo/.composio/composio (not on PATH — use full path).
...
3. Prefix all GL codes with a single quote when writing cell values (e.g., "'6090") so Sheets stores them as text, not numbers. SUMIFS string-matching will break silently if you skip this.
...
4. If CoA changed mid-project (added/renamed codes via `build-chart-of-accounts`), rewrite `config/chart-of-accounts.json` AND reset `config/prior-categorizations.json` from this run's high-confidence entries only — stale codes from old CoA misroute future transactions.
...
5. **CoA locked at categorization start.** Don't add GL codes mid-run.
...
10. **Store GL codes as text in Sheets** by prefixing `'` (e.g. `"'6090"`). Else SUMIFS miss every row.
...
- `CHART_OF_ACCOUNTS.md` — default CoA (income, COGS, expense, equity, transfer accounts)
...
- **CoA drift mid-project** — if add or rename GL codes mid-project, rewrite `chart_of_accounts.json` AND reset `prior_categorizations.json` from this run only. Stale codes WILL misroute future transactions.