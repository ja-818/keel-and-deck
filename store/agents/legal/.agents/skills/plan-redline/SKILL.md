---
name: plan-redline
description: "- \"Draft redline strategy for {counterparty} contract\" / \"what push back on?\" / \"prioritize redlines  -  limited leverage\"."
version: 1
tags: [legal, plan, redline]
category: Legal
featured: yes
image: scroll
inputs:
...
- After `review-contract` (mode=full) surfaces Yellow + Red clauses, founder needs negotiation sequence.
...
2. **Read upstream review.** Find matching `contract-reviews/{counterparty-slug}-{YYYY-MM-DD}.md`. Missing → stop, tell founder run `review-contract` (mode=full) first. Pull full clause table (Green / Yellow / Red + current text + market standard).
...
12. **Summarize to user.** One paragraph: must-have count + top one + ask/offer framing one line + next move. Drop path.
