...
description: "Use when you say 'where are my signatures' / 'log this executed agreement' / 'what's due soon' / 'Monday legal review'  -  track `scope` you pick: `signatures` watch signing platform (DocuSign / PandaDoc / HelloSign) for outstanding docs, draft reminders · `counterparties` log executed agreements to `counterparty-tracker.json` · `deadlines` refresh canonical calendar (Delaware March 1, 83(b) 30-day, 409A 12-month, DSR 30/45-day) · `weekly-review` roll up what shipped this week."
version: 1
tags: [legal, track, legal]
category: Legal
featured: yes
image: scroll
...
- Implicit: chained from `review-contract` (any mode) to `counterparties` when contract hit executed status; from scheduled routines for `weekly-review` + `deadlines`; from `triage-legal-inbox` when detect executed-copy attachment for `counterparties`.

## Ledger fields I read

Read `config/context-ledger.json` first.

- `universal.legalContext` + `context/legal-context.md`  -  recommended not required. Enrich `weekly-review` with standing context. If missing + `weekly-review` scope, run `define-legal-context` skill or proceed with note.
...
3. **Branch on `scope`.**
   - `signatures`:
     ...
     4. Write `signature-status/{YYYY-MM-DD}.md`  -  three sections: Outstanding (+ reminders) / Recently executed (+ paths) / Stalled (> 14 days open  -  recommend outreach or withdraw). For each executed envelope, recommend chain to `track-legal-state` scope=counterparties to log.
   - `counterparties`:
...
5. **Summarize.** One paragraph  -  top signal from this scope + path.
