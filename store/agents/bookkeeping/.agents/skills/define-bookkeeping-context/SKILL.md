---
name: define-bookkeeping-context
description: "Use when the user says 'set up the books' / 'onboard us as a client' / 'draft the bookkeeping brief' / 'capture our opening balances'  -  I interview the founder once and write `context/bookkeeping-context.md` plus the relevant fields on `config/context-ledger.json`. Sub-mode `mode=opening-balances` captures the opening trial balance to `config/opening-trial-balance.json`. Every other skill in this agent reads the brief before doing substantive work."
version: 1
tags: [bookkeeping, define, bookkeeping]
...
Agent OWNS `context/bookkeeping-context.md`. No other skill writes it. This skill creates/updates it and hydrates matching fields on `config/context-ledger.json`. Existence unblocks rest of agent  -  `process-statements`, `run-monthly-close`, `prep-investor-financials`, etc. refuse substantive work against missing brief.

Draft-only: never file, post to GL, connect to bank. Collect facts.
...
- `mode=opening-balances`  -  "capture our opening trial balance" / "load our starting balances from this spreadsheet" / "book the opening TB from our prior accountant".
- Called implicitly by other skill needing brief and finding it missing  -  only after confirming inline with user.
...
3. **Collect missing (one targeted question per gap).** For each required ledger field not set, ask ONE question with modality hint (connected app > file > URL > paste) and write answer atomically before moving on. Required fields for full brief:
   ...
   - `universal.suspenseCode`  -  default `{"code":"99999","name":"Suspense"}` unless founder's prior CoA uses different code.
   - `domains.banks.accounts[]`  -  per bank account, credit card, Stripe, payment processor: `last4`, `type`, `bank`, `glCode` (blank OK if CoA doesn't exist yet), `glName`. Prefer Composio connection (Plaid / banking category) over manual list.
   ...
   - `domains.investors`  -  cadence, `anchorKpis[]` (e.g., ARR, Gross Margin, Burn, Runway), `format`.
...
4. **Capture founder-specific hard nos.** One open question: "any things I should never touch without explicit sign-off?" Common: stock-based comp (no 409A input), revrec on non-standard contracts, related-party transactions, crypto. Record verbatim.
...
5. **Draft brief (~400-700 words, opinionated, direct).** Structure, in order:
   ...
   3. **Bank accounts & cards**  -  grouped by `last4`; each with bank, type, GL code, GL name. Flag account without GL code as `TBD  -  set when CoA exists`.
   ...
   9. **Hard nos**  -  workspace-level ("never post to GL, never move money, never file anything") + founder-specific.
...
6. **`mode=opening-balances` branch.** If triggered:
   ...
   - Validate every `glCode` exists in `config/chart-of-accounts.json`. If CoA absent, stop and ask user to run `build-chart-of-accounts` first (or run inline). NEVER invent GL code here.
...
9. **Summarize to user.** One short paragraph: what captured, what still TBD, exact next move (usually: "run `build-chart-of-accounts` next, then drop bank statements into `statements/_inbox/`").
