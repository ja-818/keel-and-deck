...
- `universal.company.stage`  -  sets power/MDE defaults (early stage = larger MDE ok).
- `domains.data.warehouse`  -  which warehouse to query (Postgres / BigQuery / Snowflake via Composio).
- `domains.data.metrics`  -  metric registry, for anomaly baselines + thresholds.
- `domains.data.schemas`  -  table shapes + freshness expectations for DQ.

Missing → ONE modality-ranked question → write ledger → continue.
