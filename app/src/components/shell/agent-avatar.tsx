...
/** Mini avatar — bare colored Houston helmet, no background.
 *  Accepts raw hex or semantic color ID — resolves automatically. */
export function AgentMiniAvatar({ color, size = 18 }: { color?: string; size?: number }) {
  const resolved = resolveAgentColor(color);
  return <HoustonHelmet color={resolved} size={size} />;
}

/** Bare Houston helmet in any color — no background, just the icon. */
export function HoustonHelmet({ color = "#9b9b9b", size = 14 }: { color?: string; size?: number }) {
  return (
    <svg
